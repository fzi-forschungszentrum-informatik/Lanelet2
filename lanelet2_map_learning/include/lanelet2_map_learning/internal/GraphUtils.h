...
struct GetTarget<true> {
  using T = FilteredMapGraph::vertex_descriptor;
  template <typename Id, typename Graph>
...
class OriginalGraphFilter {
 public:
  ...
  OriginalGraphFilter(const GraphType& g, bool withLaneChange)
      : g_{&g}, filterMask_{RelationType::Successor | RelationType::Conflicting} {
    if (withLaneChange) {
      filterMask_ |=
  ...
  bool operator()(const GraphTraits::edge_descriptor& v) const {
    ...
    return (edge.relation & filterMask_) != RelationType::None;
  }
...
class EdgeRelationFilter {
 public:
  ...
  bool operator()(FilteredMapGraph::edge_descriptor e) const {

...
#include "lanelet2_map_learning/internal/ShortestPath.h"
...
class ConfigurationInitialization : public ::testing::Test {
 public:
  ...
  MapGraph::Configuration MapGraphConf;
};

TEST_F(ConfigurationInitialization, EmptyConfig) {                                             // NOLINT
  EXPECT_NO_THROW(MapGraph::build(*emptyMap, *trafficRules, costPtrs, MapGraphConf));  // NOLINT
}

TEST_F(ConfigurationInitialization, Config) {                                                  // NOLINT
  MapGraphConf.emplace(std::make_pair(MapGraph::ParticipantHeight, Attribute("2.")));  // NOLINT
  EXPECT_NO_THROW(MapGraph::build(*emptyMap, *trafficRules, costPtrs, MapGraphConf));  // NOLINT
}

TEST_F(ConfigurationInitialization, NoConfig) {                              // NOLINT
  EXPECT_NO_THROW(MapGraph::build(*emptyMap, *trafficRules, costPtrs));  // NOLINT
}

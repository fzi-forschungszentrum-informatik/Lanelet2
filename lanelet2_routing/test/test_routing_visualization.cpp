...
TEST_F(GermanVehicleGraph, GraphVizExport) {  // NOLINT
  ...
  EXPECT_NO_THROW(graph->exportGraphViz(file));
  RelationTypes excluded{RelationType::Conflicting};
  EXPECT_NO_THROW(graph->exportGraphViz(fileWithExcluded1, excluded));
  excluded.push_back(RelationType::AdjacentLeft);
  excluded.push_back(RelationType::AdjacentRight);
  EXPECT_NO_THROW(graph->exportGraphViz(fileWithExcluded2, excluded));
}

TEST_F(GermanVehicleGraph, GraphVizExportError) {  // NOLINT
  EXPECT_THROW(graph->exportGraphViz("", RelationTypes()), lanelet::InvalidInputError);
  EXPECT_THROW(graph->exportGraphViz("/bla", RelationTypes()), lanelet::ExportError);
}
...
TEST_F(GermanVehicleGraph, GraphMLExport) {  // NOLINT
  ...
  EXPECT_NO_THROW(graph->exportGraphML(file));
  RelationTypes excluded;
  excluded.push_back(RelationType::Conflicting);
  EXPECT_NO_THROW(graph->exportGraphML(fileWithExcluded1, excluded));
  excluded.push_back(RelationType::AdjacentLeft);
  excluded.push_back(RelationType::AdjacentRight);
  EXPECT_NO_THROW(graph->exportGraphML(fileWithExcluded2, excluded));
}

TEST_F(GermanVehicleGraph, GraphMLExportError) {  // NOLINT
  EXPECT_THROW(graph->exportGraphML("", RelationTypes()), lanelet::InvalidInputError);
  EXPECT_THROW(graph->exportGraphML("/place_that_doesnt_exist", RelationTypes()), lanelet::ExportError);
}

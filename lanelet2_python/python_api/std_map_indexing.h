...
class final_std_map_derived_policies
    : public std_map_indexing_suite<Container, NoProxy, final_std_map_derived_policies<Container, NoProxy> > {};
}  // namespace detail
...
template <class Container, bool NoProxy = false,
          class DerivedPolicies = detail::final_std_map_derived_policies<Container, NoProxy> >
class std_map_indexing_suite
    : public indexing_suite<Container, DerivedPolicies, NoProxy, true, typename Container::value_type::second_type,
                            typename Container::key_type, typename Container::key_type> {
 public:
  ...
  static bp::list keys(Container const& x) {
    bp::list t;
    for (typename Container::const_iterator it = x.begin(); it != x.end(); it++) t.append(it->first);
    return t;
  }
  // return a list of values
  static bp::list values(Container const& x) {
    bp::list t;
    for (typename Container::const_iterator it = x.begin(); it != x.end(); it++) t.append(it->second);
    return t;
  }
  ...
  struct make_transform_impl {
    ...
    static iterator begin(const Container& m) { return boost::make_transform_iterator(m.begin(), Transform()); }
    static iterator end(const Container& m) { return boost::make_transform_iterator(m.end(), Transform()); }
  ...
  static typename mpl::if_<is_class<data_type>, data_type&, data_type>::type get_data(
  ...
  static bool compare_index(Container& container, index_type a, index_type b) { return container.key_comp()(a, b); }
  ...
  static void extension_def(Class& cl) {
    ...
    typedef typename mpl::if_<is_class<data_type>, return_internal_reference<>, default_call_policies>::type
    ...
    class_<value_type>(elem_name.c_str())
        ...
        .add_property("first", &DerivedPolicies::get_key, "K.first() -> the first item in this pair.\n")
    ...
    cl
        ...
        .def("get", dict_get,
             dict_get_overloads(args("default_val"), "D.get(k[,d]) -> D[k] if k in D, else d.  d defaults to None.\n"))
        .def("pop", &dict_pop)
        ...
        .def("update", &dict_update, "D.update(E) -> None.  Update D from E: for k in E: D[k] = E[k]\n")

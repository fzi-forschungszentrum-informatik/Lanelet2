    ...
    for (auto& lanelet: map.laneletLayer) {
        ...
        if(left_bound_2d_hyb.size() >= 3){
            for(size_t i=1; i<left_bound_2d_hyb.size()-1;++i){
                if(std::fabs(computeCurvature(left_bound_2d_hyb[i - 1],
                                              left_bound_2d_hyb[i],
                                              left_bound_2d_hyb[i + 1])) > 0.5){
                    issues.emplace_back(
                            Severity::Warning, Primitive::Lanelet, lanelet.id(),
        ...
        if(right_bound_2d_hyb.size() >= 3){
            for(size_t i=1; i<right_bound_2d_hyb.size()-1;++i){
                if(std::fabs(computeCurvature(right_bound_2d_hyb[i - 1],
                                              right_bound_2d_hyb[i],
                                              right_bound_2d_hyb[i + 1])) > 0.5){
                    issues.emplace_back(
                            Severity::Warning, Primitive::Lanelet, lanelet.id(),
...
double CurvatureTooBigChecker::computeCurvature(const lanelet::BasicPoint2d& p1,
